...
class _BuyOrderState extends State<BuyOrder> {
  ...
  @override
  void initState() {
    changePrice = ScriptInfoModel.price.toString();
    changeQty = ScriptInfoModel.lotSize.toString();
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    TextEditingController priceCtrl =
        TextEditingController(text: changePrice.toString());
    priceCtrl.selection =
        TextSelection.collapsed(offset: priceCtrl.text.length);
    TextEditingController qtyCtrl =
        TextEditingController(text: changeQty.toString());
                                    ...
                                    setState(() {
                                      if (priceFocus) {
                                        var tick = double.parse(
                                            ScriptInfoModel.tikTik);
                                        if (changePrice!.isNotEmpty) {
                                          changePrice =
                                              (double.parse(changePrice!) +
                                                      tick)
                                                  .toStringAsFixed(2);
                                        } else {
                                          changePrice =
                                        ...
                                        var lotSize = ScriptInfoModel.lotSize;

                                        if (changeQty!.isNotEmpty) {
                                          changeQty =
                                              (int.parse(changeQty!) + lotSize)
                                                  .toString();
                                        } else {
                                          changeQty = (int.parse("0") + lotSize)
                                              .toString();
                                        }
                                      }
                                    });
  ...
  Future placeOrder(
    ...
    try {
      ...
      http.Response response = await http.post(Uri.parse(ApiLinks.placeOrder),
          ...
          body:
              '''jData={"uid":"${ConstVariable.userId}","actid":"${ConstVariable.accId}","exch":"$exchange","tsym":"$symName","amo":"$amo","qty":"$quantity","prc":"$price","dscqty":"$diskQty","prd":"$product","trantype":"B","trgprc":"$triggerPrice","prctyp":"$priceTyp","bpprc":"$target","trailprc":"$trailingStoploss","ret":"$validityType","blprc":"$stopLoss","ordersource":"MOB"}&jKey=${ConstVariable.sessionId}''');
